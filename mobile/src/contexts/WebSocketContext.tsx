...
import type { WsConnectionStatus, WsEventPayload } from "@/types";
...
export function WebSocketProvider({ children }: { children: ReactNode }) {
  ...
  const connect = useCallback(() => {
    ...
    const url = `${ENV.WS_BASE}/ws?token=${encodeURIComponent(token)}`;
    let ws: WebSocket;
    try {
    ...
    (handler: WsHandler, filter: WsFilter) => {
      const wrapped: WsHandler = (event) => {
        if (
          ...
          return;
        const eventWaId =
          (event.payload.waId as string) ?? (event.payload.phone as string);
        if (filter.waId && eventWaId && eventWaId !== filter.waId) return;
        if (
          filter.stageId != null &&
      ...
      return () => {
        handlersRef.current.delete(wrapped);
      };
    },
    [],
  );
  ...
  const value = useMemo<WebSocketContextValue>(
    () => ({ status, lastHeartbeat, subscribe, subscribeFiltered }),
    [status, lastHeartbeat, subscribe, subscribeFiltered],
  );
